	 ...
	 BEGIN TRY
		...
		SELECT 
			...
			CASE WHEN UPPER(TRIM(cst_material_status)) = 'S' THEN 'Single'
				...
				ELSE 'n/a'
			END AS marital_status, -- Normalize martial status values to readable format
		...
		SELECT 
			   ...
			   CAST(prd_start AS DATE) AS prod_start,
			   CAST(
					 LEAD(prd_start) OVER(PARTITION BY prd_key ORDER BY prd_start)  -  1 
					 AS DATE
